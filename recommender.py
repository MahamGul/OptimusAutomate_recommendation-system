...
def hybrid_recommend(movie, content_sim, content_idx, movies_df,
    ...
    """Blend content + collab scores."""
    c_results = {r["title"]: r["score"] * 0.5 for r in
                 recommend_content(movie, content_sim, content_idx, movies_df, top_n * 2)}
    cf_results = {r["title"]: r["score"] * 0.5 for r in
                  recommend_collab(movie, collab_sim, collab_idx, matrix, top_n * 2)}
    ...
    for title, score in top:
        ...
        results.append({"title": title, "score": float(score), "genres": genres})
    return results
